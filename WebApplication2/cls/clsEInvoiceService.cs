...
{
    public class clsEInvoiceService
    {
        public EInvoiceSubmitResult SubmitEInvoice(int CompanyID, string InvoiceGuid, string FinancingGuid, string ReturnInvoiceNumber)
        {
            SqlTransaction trn;
            ...
            try
            {
                if (Simulate.String(InvoiceGuid) == "" && Simulate.String(FinancingGuid) == "")
                {
                    trn.Rollback();
                    return new EInvoiceSubmitResult
                    {
                        ...
                        HttpStatusCode = 400,
                        ID = "",
                        Message = "Please provide InvoiceGuid or FinancingGuid.",
                        QR = null
                    };
                }
                ...
                if (dtEinvoiceConfiguration.Rows.Count == 0)
                {
                    trn.Rollback();
                    con.Close();
                    return new EInvoiceSubmitResult
                    {
                        ...
                        HttpStatusCode = 400,
                        ID = "",
                        Message = "E-invoice Configuration not complete",
                        QR = null
                    };
                }
                if (Simulate.String(InvoiceGuid) != "")
                {
                    ...
                    } else if (Simulate.String( dtHeader.Rows[0]["EInvoiceQRCode"])!="") {
                    ...
                    "", "", CompanyID, -1, -1, -1, -1, trn);
                        ...
                        clsEInvoiceXMLCreator.createLine(Simulate.String(dtCurrency.Rows[0]["Code"]),//"JOD"
                         (i + 1) //index
                        , Simulate.decimal_(dtDetails.Rows[i]["qty"])//Quantity
                        , Simulate.decimal_(dtDetails.Rows[i]["PriceBeforeTax"])//Price before Tax
                        , Simulate.decimal_(dtDetails.Rows[i]["TaxAmount"])//TaxAmount
                        , Simulate.decimal_(dtDetails.Rows[i]["TotalLine"])//PriceAfterTax
                         , Simulate.decimal_(dtDetails.Rows[i]["taxid"]) == 0 ? ItemTaxCategory.Zero : ItemTaxCategory.Taxable
                        , (Simulate.decimal_(dtDetails.Rows[i]["TaxPercentage"]) * 100)//Taxpercenage  
                        , Simulate.String(dtDetails.Rows[i]["ItemName"]) // itemName
                        , Simulate.decimal_(dtDetails.Rows[i]["DiscountAfterTaxAmountAll"])//Total Discount
                        ));
                    ...
                    else
                    {
                       ...
                       // return Ok(new { ID = "", Message = "Invoice Type Not Supported." });
                        return new EInvoiceSubmitResult
                        {
                            ...
                            HttpStatusCode = 400,
                            ID = "",
                            Message = "Invoice Type Not Supported.",
                            QR = null
                        };
            ...
            catch (Exception ex)
            {
                ...
                //   return EInvoiceSubmitResult("rollback area:" + ex.Message);
                return new EInvoiceSubmitResult
                {
                    Success = false,
                    HttpStatusCode = 500,
                    ID = "",
                    Message = "rollback area:" + ex.Message,
                    QR = null
                };
            }
            finally
...
public class EInvoiceSubmitResult
{
    ...
    // Optional: let controller decide status
