...
when (id=@ArAccountID   or IsSubLedger =1  )  then 
999
when (id=@BankAccountID    )  then 
999
when (id=@CashAccountID   )  then 
...
select 
    a.ID as AccountID,
    a.AccountNumber + '-' + FORMAT(jvd.SubAccountID, '000000;-000000') as AccountNumber,
    bp.AName,
    bp.EName,
   jvd.SubAccountID as SubAccountID,
    0 as ChildCount
from tbl_Accounts a
left join tbl_JournalVoucherDetails jvd on jvd.AccountID = a.ID
left join tbl_BusinessPartner bp on jvd.SubAccountID = bp.ID
where a.CompanyID = @companyid and
   a.IsSubLedger = 1 
  and a.ID not in (@ARAccountID, @APAccountID, @BankAccountID, @CashAccountID)
 
  and jvd.CompanyID = @companyid
group by a.ID, a.AccountNumber, jvd.SubAccountID, bp.AName, bp.EName
) as q  
) as qq where qq.OpeningBalance <>0 or qq.EndingBalance <>0 or qq.Debit<>0 or qq.Credit <>0
order by qq.AccountNumber asc";
