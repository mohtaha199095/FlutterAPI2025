       ...
       new SqlParameter("@EName", SqlDbType.NVarChar,-1) { Value = EName },
        ...
        public bool DeleteHRContractTypeByID(int Id, int CompanyID)
        {
            try
            {
                ...
                int A = clsSQL.ExecuteNonQueryStatement(@"delete from tbl_HRContractType where id=@Id AND CompanyID=@CompanyID", clsSQL.CreateDataBaseConnectionString(CompanyID), prm);
