...
namespace WebApplication2.cls
{
    ...
    {
        public DataTable SelectJobTitleByID(int Id, string AName, string EName,  int CompanyID)
        {
            try
            {
                SqlParameter[] prm =
      ...
      new SqlParameter("@AName", SqlDbType.NVarChar,-1) { Value = AName },
       new SqlParameter("@EName", SqlDbType.NVarChar,-1) { Value = EName },
        ...
        new SqlParameter("@CompanyID", SqlDbType.Int) { Value = CompanyID },
                ...
                DataTable dt = clsSQL.ExecuteQueryStatement(@"select * from tbl_JobTitle where (id=@Id or @Id=0 ) and  
        ...
        public bool DeleteJobTitleByID(int Id, int CompanyID)
        {
            try
            {
                ...
                int A = clsSQL.ExecuteNonQueryStatement(@"delete from tbl_JobTitle where id=@Id AND CompanyID=@CompanyID", clsSQL.CreateDataBaseConnectionString(CompanyID), prm);
        ...
        public int InsertJobTitle(string AName, string EName,  int CompanyID, int CreationUserId, SqlTransaction trn = null)
        {
            try
            {
                SqlParameter[] prm =
                ...
                string a = @"insert into tbl_JobTitle(AName,EName,CompanyID,CreationUserId,CreationDate)
        ...
        public int UpdateJobTitle(int ID, string AName, string EName,  int ModificationUserId, int CompanyID)
        {
            try
            {
                ...
                int A = clsSQL.ExecuteNonQueryStatement(@"update tbl_JobTitle set 
                       AName=@AName,
                       EName=@EName,
